...
.text:
	...
	main:
		...
		add $s0, $0, $zero	#location to save a character
	...
	increment_to_next:
		jal subprogram_2
		addu $s1, $0, 1		#adds to starting index
		addu $s2, $0, 1		#adds to ending index
	...
	subprogram_2:
		...
		subu $t0, $s2, $s1	#gets length of the (sub) string by subtrating last from first index
		bgt $t0, 8, too_large	#has too many bytes if length (shift position) is greater than 8
		...
		sw $t0, 16($sp)		#stores the position to the stack as well.
